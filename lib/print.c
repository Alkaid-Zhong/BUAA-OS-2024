...
void vprintfmt(fmt_callback_t out, void *data, const char *fmt, va_list ap) {
	...
	for (;;) {
		...
		case 'P':
			...
			z = z < 0 ? -z : z;
			out(data, &lp, 1);
			print_num(out, data, num < 0 ? -num : num, 10, num < 0, width, ladjust, padc, 0);
			out(data, &dh, 1);
			print_num(out, data, num2 < 0 ? -num2 : num2, 10, num2 < 0, width, ladjust, padc, 0);
			out(data, &dh, 1);
		...
		case 'D':
			...
			//(fmt_callback_t out, void *data, unsigned long u, int base, int neg_flag, int length, int ladjust, char padc, int upcase)
			if (num < 0) {
				neg_flag = 1;
